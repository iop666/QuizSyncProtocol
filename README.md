...
| `spec/` | 规范正文，每个域一个文件 —— 见 [`spec/README.md`](spec/README.md) | 待写（Phase 1） |
| `schema/` | JSON Schema 2020-12：`envelope.json` + 实体 + 每种消息 | 待写（Phase 1） |
| `examples/` | 每条消息一组 request/response 示例（含错误示例） | 待写（Phase 1） |
| `versions/` | `v1-input/`（现状输入材料，**已就位**）、`v1-snapshot.md`、`v2-draft/`、`compat-matrix.md`、`CHANGELOG.md` | 部分 |
| `conformance/` | 语言无关的一致性向量（配对 / 鉴权 / 上传 / 任务 / sync 收敛 / 错误）+ 回放说明 | 待写（Phase 1） |
...
- [ ] `versions/v1-snapshot.md`：按**现有实现**反推的现状规范（修掉文档-实现分叉）
- [ ] `spec/` 11 份 + `schema/` + `examples/`
- [ ] `conformance/vectors` 首批 6 组 + Dart 参考回放器跑通
- [ ] CI：JSON Schema 校验 examples + 向量格式校验
